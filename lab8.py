...
def register():
    if request.method == 'GET':
        return render_template('lab8/register.html', error='')
    ...
    if not login_form or login_form.strip() == '':
        return render_template('lab8/register.html', 
                             error='Введите логин (поле не должно быть пустым)')
    
    if not password_form or password_form.strip() == '':
        return render_template('lab8/register.html', 
                             error='Введите пароль (поле не должно быть пустым)')
...
def login():
    ...
    user = users.query.golter_by(login_form).first()
